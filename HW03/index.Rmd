---
title: "<insert> <img src=\"https://nlepera.github.io/sta551/HW01/img/penguin_cute.png\" style=\"float: right; width: 12%\"/>"
subtitle: "<insert>"
author:
- name: Natalie LePera
...
Once imputed, the data was then analysed for any potential skew and subsequently normalized utilizing a min-max equation.  This normalization was implemented to further prepare the data for use in predictive modeling by reducing any skew and ensuring normal distributions to ensure the assumptions of regression were met.  The normalized data was then analyzed for redundancies and the high impact variables were identified for retention through RFE feature selection.  Once fully pruned, the data was then subjected to k-Nearest Neighbor cluster based feature extraction.
...
```{r, fig.width= 10, fig.heigt = 8, out.height="100%"}
...
In order to determine the best linear regression prediction model for live incoming plane delay prediction and overall airline functional planning to reduce average delay times, multiple models were created. The following models were developed:
...
## PCA Model Analysis & Evaluatiom

Once selected as the best fit linear regression prediction model, the PCA model (Model 2) was re-evaluated utilizing the test data rather than the training data to ensure accuracy and ensure the model was not over-fitted. As a reminder, Model 2 consists of a linear regression model that set PC1, and the created k-Means Cluster ID as the independent variables with Overall Flight Delay as the dependent variable.  As a part of the development of each PCA, the original variables were linearly combined to create these principal components, effectively setting each PC to be a unique combination of 6 normalized variables (Airport Distance, Number of Flights, Weather, Support Crews Available, Late Incoming Plane Arrival, & Baggage Loading Time).  

As identified in `Table 17.a` and `Table 17.b` below, each dependent variable returned a statistically significant p value of well below 0.05 when model 2 was run utilizing the test data as well as the.  This has ensured that all variables present in the regression model are significant to the prediction model and are not contributing noise or redundancy to the model. 

```{r}
lm.pca.test <- lm(arr_delay ~ PC1 + cluster, data = test.data)

lm.pca <- lm(arr_delay ~ PC1 + cluster, data = final.delay)

kable(summary(lm.pca.test)$coefficients, col.names = c("Estimate", "Std. Error", "t value", "Pr(> I t I)"), caption = "Table 17.a:
      <br>Regression coefficients for linear regression prediction model 2 (PCA) run on <b>test data</b>") %>% 
  kable_styling()

kable(summary(lm.pca)$coefficients, col.names = c("Estimate", "Std. Error", "t value", "Pr(> I t I)"), caption = "Table 17.b:
      <br>Regression coefficients for linear regression prediction model 2 (PCA) run on <b>full data</b>") %>% 
  kable_styling()
```
...
Arrival \ Delay \ _{test} &\sim 0.0961028x_1 \ + \ 0.0307025x_2 \ + \ 0.3497221 \\ \\
Arrival \ Delay \ _{full} &\sim 0.0915130x_1 \ + \ 0.0195626x_2 \ + \ 0.3627055	
\end{align*}
$$

As demonstrated by the above equations and `Tables 17.a & b` the regression equations from the test and full data prove to be similar, underlining the developed model 2 serves well as a prediction model with minimal over fitting. Additionally, as captured below in `Table 18` the calculated $R^2$ values for model 2 as fit to both the test data and full final delay data indicate model 2 is a good fit for Overall Flight Delay prediction. 

```{r}
rsq.lin <- data.frame(
  test = summary(lm.pca.test)$r.squared,
  full = summary(lm.pca)$r.squared
)

kable(rsq.lin, col.names = c("Test Data", "Full Final Delay Data"), caption = "Table 18:
      <br>$R^2$ value of model 2 on both test data & full final delay data") %>% 
  kable_styling()
```
